  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        <div className="border border-white/10 rounded-xl max-h-96 overflow-y-auto custom-scrollbar bg-slate-800/30">
          {isLoading ? (
            ...
            <div className="divide-y divide-white/5">
              {filteredUsers.map((user: any) => {
                ...
                return (
                  <label 
                    key={user.id} 
                    className={`flex items-center gap-3 p-3 cursor-pointer hover:bg-white/5 transition-colors ${isSelected ? 'bg-brand-500/5' : ''}`}
                  >
                    <div className="relative flex items-center">
                      ...
                      />
                      <div className="w-5 h-5 rounded border border-white/20 flex items-center justify-center bg-slate-900 peer-checked:bg-brand-500 peer-checked:border-brand-500 transition-colors">
                        {isSelected && (
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400">{user.email} • {user.role?.displayName || user.role?.name}</p>
                    </div>
                  </label>
                );
              })}
        ...
        <div className="flex justify-between items-center pt-4 border-t border-[rgba(255,255,255,0.1)]">
          <p className="text-sm text-slate-400">
            <span className="font-medium text-white">{selectedUserIds.size}</span> users selected
          </p>
