...
export const Sidebar = () => {
  ...
  return (
    <aside className={`bg-slate-800/40 backdrop-blur-xl border-r border-white/10 flex flex-col transition-all duration-300 z-40 relative ${sidebarCollapsed ? 'w-[70px]' : 'w-64'}`}>
      <div className="h-16 flex items-center px-5 border-b border-white/10 shrink-0">
        <div className="flex items-center gap-2 text-brand-400 font-bold text-lg overflow-hidden whitespace-nowrap">
        ...
        {navItems.map((item) => {
          ...
          return (
            <NavLink 
              key={item.to} 
              to={item.to} 
              className={({ isActive }) => `flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors mb-1 ${isActive ? 'bg-brand-500/15 text-brand-400 font-medium' : 'text-slate-400 hover:bg-white/5 hover:text-slate-100'}`}
      ...
      <div className="p-4 border-t border-white/10 flex justify-end shrink-0">
        <button className="p-2 rounded-lg text-slate-400 hover:text-slate-100 hover:bg-slate-700/50 transition-colors" onClick={toggleSidebar}>
